...
`{rix}` is an R package that leverages [Nix](https://nixos.org/), a package manager focused on reproducible builds. With Nix, you can create project-specific environments with a custom version of R, its packages, and all system dependencies (e.g., `GDAL`). Nix ensures full reproducibility, which is crucial for research and development projects.
...
While Nix has a steep learning curve, `{rix}` simplifies creating Nix expressions, which define reproducible environments. You can work interactively in IDEs like RStudio or VS Code, or use Nix in CI/CD workflows. Nix includes nearly all CRAN and Bioconductor packages, with the ability to install specific package versions or GitHub snapshots.

For isolation, `{rix}` includes a `rix_init()` function to prevent conflicts with system-installed R versions, offering better control over your environment.

## Quick Start for Returning Users

<details><summary>Click to expand</summary>

If you're already familiar with Nix and `{rix}`, install Nix using the [Determinate Systems installer](https://determinate.systems/posts/determinate-nix-installer):
