...
run Nixpkgs. Easiest is to get your R system set up via Nixpkgs, and then
install {rix}. Feel free to check out the instructions at the bottom for further 
details on Windows.
...
will be for single-user installations if you have `systemd` disabled (default).

To make life easy, we recommend that you install {rix} using nix, which also
equips you with the current R version available from Nixpkgs and RStudio.
Please note that we currently manually update `default.nix` on GitHub. If 
you want the latest development version of {rix}, we recommend to use {remotes}
