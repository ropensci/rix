...
On Linux, once Nix is installed, all the software that will be installed will be
...
your root partition is limited, I advise you to mount the `/nix` folder on
...
Suppose that you have the program `sl` installed on your system, and suppose you
...
It is also possible to completely isolate an environment built with Nix using
the provided `init()` function. It is also possible to run an arbitrary R
function or shell command inside a *sub-shell* with a completely different environment
right from an interactive R session. You can read more on this in the vignette
titled [blal].
...
*It should be noted that as of now, RStudio isn't available for macOS through Nix. The
`default.nix` generated with the above call won't build on macOS. If you're running
macOS, you consider reading this [macOS-specific vignette]().*
...
vignette](). In case of doubt, run `nix-build` from your usual terminal
application.
...
(note the value provided to the `ide` argument, this will work on any OS as long
as you have VS Code installed on it).
...
because VS Code requires this package to interact with R. This will not install
a Nix environment-specific version of VS Code. Now, instead of typing `rstudio`
in the Nix shell of your environment, type `code` and this will start the Visual
Studio Code you usually use.
...
If you're on macOS, read the [macOS-specific vignette](). 

If you want to keep using your main R environment, but wish to still be able
to run specific functions with older packages or older versions of R for example,
you might be interested in the `with_nix()` function.
