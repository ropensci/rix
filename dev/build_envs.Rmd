---
title: "Functions to build environments"
...
This function can evaluate an shell or R expression in Nix.
...
recurse_find_check_globals <- function(expr, args_vec) {
    ...
    repeat {
      # first round
      names_globals <- names(globals_lst) %in%
        c("globalenv_fun", "globalenv_other", "env_other", "env_fun", "pkgs")
      first_globals <- all(names_globals) && length(names_globals) != 0L
      ...
      if (first_globals) {
        ...
        # starting from second round of repeat
      ...
      result_list <- c(list(globals_lst), globals_lst_new) # list(globals_lst)
      ...
      globals_null <- vapply(globals_lst, function(x) all(is.null(x)), 
