...
#' fetchzips Downloads and installs a packages hosted in the CRAN archives. Wrapper around `fetchzip()` to handle multiple packages
...
fetchzips <- function(archive_pkgs){
  ...
  if(is.null(archive_pkgs)){
    ...
    stop("There is something wrong with the input. Make sure either a sinle package name, or an atomic vector of package names, for example c('dplyr@0.8.0', 'tidyr@1.0.0').")
  }
...
rix <- function(r_ver = "current",
  ...
  # get archive_pkgs
  archived_pkgs <- r_and_archive_pkgs$archive_packages
