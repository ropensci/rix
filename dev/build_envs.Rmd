...
with_nix <- function(expr,
  ...
  if (exec_mode == "non-blocking") poll_non_blocking(cmd, proc)
  
  if (exec_mode == "blocking") poll_blocking(cmd, proc)
...
#' @noRd
poll_blocking <- function(cmd, proc) {
  status <- proc$status
    if (status == 0L) {
      cat(paste0("\n==> ", sys::as_text(proc$stdout)))
      cat("\n==> `expr` succeeded!")
...
#' @noRd
poll_non_blocking <- function(cmd, proc) {
  cat(paste0("\n==> Process ID (PID) is ", proc, "."))
  cat("\n==> Receiving stdout and stderr streams...\n")
  status <- sys::exec_status(proc, wait = TRUE)
  if (status == 0L) {
