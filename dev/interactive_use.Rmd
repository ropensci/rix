...
*If you're on macOS, we recommend you read [the macOS-specific vignette instead]().
This vignette will assume you're on Linux or on WSL2.*
...
regarding this, refer to the
[vignette discussing `with_nix()`](https://b-rodrigues.github.io/rix/articles/running-r-or-shell-code-in-nix-from-r.html).

## Scenario 1: you installed R and RStudio using the usual installers

*Important: this vignette assumes you are using a Linux distribution or WSL2!
For macOS, refer to [the macOS-specific vignette]()!*
...
interpreter installed with Nix.
...
Navigate to that folder using your terminal, and then run `nix-shell`. You will
then be *dropped into* a Nix shell. From there you can type `rstudio` to run
this project specific version of R and RStudio with all the required packages
for this project, in this case `{dplyr}` and `{ggplot2}`. You can then work on
it as usual.
...
Notice the `ide = "code"` argument. This will not install VS Code from Nix,
but will install the `languageserver` package that is needed for using R
with VS Code. If you want to also install a project-specific version of
VS Code, then use `system_pkgs = "vscode"` or `system_pkgs = "vscodium"`
if you prefer VS Codium over VS Code. For any other editor such as Emacs
or Vim, set `ide = "other"`. Also use "other" if you want to run scripts
non-interactively, for example on a CI/CD service.
