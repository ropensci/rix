...
dramatically improving the odds of success. To set a GitHub PAT for the current
session, simply run:
...
Of course, you first need to create a [GitHub PAT
...
will actually pull another version of these packages. So we very highly recommend
to edit the `default.nix`, and replace mentions of `HEAD` with an
