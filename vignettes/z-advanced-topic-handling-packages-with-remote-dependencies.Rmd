...
Again you would need to manually remove the duplicates from the `default.nix` file before `nix-build` will work.
Be aware of dependency issues: for example the older version of package `C` might be imcompatbile with package `A`.
