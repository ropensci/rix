...
will not benefit from the Nix-specific fixes that need to be applied to its
source code for it to build successfully. In order to install the version of
`{httr2}` from CRAN which will build successfully, change the `default.nix`
manually to this (essentially remove the definition of `{httr2}` and put it
as a `propagatedBuildInput` to `{gh}`):
...
and then try to build it will work. In the second, manually edited expression,
`{httr2}` will now build successfully because Nix is instructed to build the
CRAN version by applying [this
...
revision. If instead the remotes are listed like this:
