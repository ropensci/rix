...
It might look like Nix is an alternative to Docker, but that's not really
the case. Docker is a containerisation tool, while Nix is a package manager.
You can use Nix in such a way that you don't need Docker anymore, but if you're
already invested in Docker, you don't have to abandon it and still benefit
from Nix.
...
Here is the code a simple Shiny app (it's the K-means app from the Shiny
examples gallery):
