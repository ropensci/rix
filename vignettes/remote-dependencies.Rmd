...
custom Git platforms such as Forgejo, Gitea, cgit, and other Git hosting
services. The usage is identical to GitHub and GitLab - simply provide the full
...
This example installs a package from codefloe.com, which runs Forgejo. `{rix}`
automatically detects the Git hosting platform and constructs the appropriate
archive download URL. Most Git hosting platforms follow either the GitHub-style
archive URL format (`/archive/<commit>.tar.gz`) or the GitLab-style format
(`/-/archive/<commit>.tar.gz`), and `{rix}` will use the GitHub-style format by
default for unknown platforms, which works for most self-hosted Git instances.
