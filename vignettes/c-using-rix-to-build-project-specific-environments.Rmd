...
- `r_ver`: the version of R required. Use "latest-upstream" for the latest version, and use `available_r()` to check which versions are available;
- `date`: instead of `r_ver`, use `date` to specify a date. This date corresponds to a CRAN
   snapshot of that day. Current R versions and latest release of BioConductor at that date
   are also provided. Check available dates using `available_dates()`;
...
expressions, the expression will use our `rstats-on-nix` fork which includes
