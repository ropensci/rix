...
Essentially this means that if you start a project with `{rix}` using `"latest-upstream"`
...
again, and check that the `/etc/nix/nix.conf` file has a line that looks like
`trusted-users = root YOUR_USERNAME`. If not, add it using admin/root privileges.
