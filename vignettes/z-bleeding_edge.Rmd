...
provide the `r_ver = "bleeding_edge"` and `r_ver = "frozen_edge"` options as
well as `r_ver = "bioc_devel"` (if you also need development versions of
Bioconductor as well) that makes it possible to use the most recent packages for
your environment, but at a cost. You must be aware of this cost which we detail
in the next section.
