...
### Use the Determinate Systems installeron Linux and Windows
...
Instead of using any system package manager, simply open a terminal and run
...
(so not Nix, as decribed before), you can install the `{rix}` package as usual
as well. Because `{rix}` is not yet on CRAN, the easiest way is to install it
from its r-universe:
