...
them manually. Note that this is impure and you should prefer using packages available in nixpkgs whenever possible.
This solution uses a `shellHook` to set up the Python environment based on a requirements.txt file.
You could use `uv`, which is 10-100x faster than pip and includes a lock file.
The rix call needs to include `uv` while the python installation can be handled by `uv`.
...
in `requirements.txt`. We recommend specifying explicit versions (e.g., `scanpy==1.11.4`).
...
and install the packages listed in `requirements.txt` within the nix environment. This will take place each time `nix-shell` is called,
