...
In this case, it is still possible to use Python-specific package manager, like `uv`, 
...
And then use `uv` from your shell as you would usually, but we recommend specifying Python packages
in a `requirements.txt` file, and setting specifying explicit versions (e.g., `scanpy==1.11.4`).
...
If this seems complicated: yes, and that is actually exactly the type of problems that Nix aims to solve.
However, there are just too many Python packages to automate their inclusion into nixpkgs like how it’s 
done for R. If you can, prefer using the Python packages included in nixpkgs.
