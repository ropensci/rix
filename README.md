...
``` bash
nix-shell -p R rPackages.rix  # Start R with rix
```
...
`vignette("b1-setting-up-and-using-rix-on-linux-and-windows")` or
`vignette("b2-setting-up-and-using-rix-on-macos")` for alternative
...
`{rix}`’s functions, their signatures, arguments, and usage examples.
This enables LLMs to write correct `{rix}` code without hallucinating
...
asking it to help you write `{rix}` code.
